...
inline void mm_set_index(__m128d packed, double value, int index) {
  *((double *)&packed + index) = value;
}
inline double mm_get_index(__m128d packed, int index) {
...
VALUE descriptive_statistics_simd_2x64(VALUE self, VALUE arrays)
{
  ...
  for (int variable_index = 0; variable_index < cols; variable_index += simd_pack_size) {
    ...
    for (int simd_slot_index = 0; simd_slot_index < simd_pack_size; simd_slot_index++) {
      ...
      double min = mm_get_index(mins, simd_slot_index);
      double max = mm_get_index(maxes, simd_slot_index);
      double mean = mm_get_index(means, simd_slot_index);
      double variance = mm_get_index(variances, simd_slot_index);
      double standard_deviation = mm_get_index(standard_deviations, simd_slot_index);
...
#ifdef HAVE_XMMINTRIN_H
  // Compile SIMD version as default if the hardware supports it 
  rb_define_singleton_method(mFastStatistics, "descriptive_statistics", descriptive_statistics_simd_2x64, 1);
  // Provide the "unpacked" version as a fallback
  rb_define_singleton_method(mFastStatistics, "descriptive_statistics_unpacked", descriptive_statistics_simd_2x64, 1);
  rb_define_singleton_method(mFastStatistics, "simd_enabled?", simd_enabled, 0);
#else
  // No SIMD support:
  // Provide both "packed" and "unpacked" versions with the same algorithm
